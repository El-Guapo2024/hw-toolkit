...
from .schem_renderer import Schematic, Symbol, Wire, WireEndpoint
...
def _passive_pin_pos(sym: Symbol, which: str) -> tuple[float, float]:
    ...
    if which in ("1", "top", "left"):
        return (x, y)
    if o == "down":  return _snap_xy((x, y + L))
    if o == "up":    return _snap_xy((x, y - L))
    if o == "right": return _snap_xy((x + L, y))
    if o == "left":  return _snap_xy((x - L, y))
