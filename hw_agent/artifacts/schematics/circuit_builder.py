...
Example:
    from hw_agent.artifacts.schematics.circuit_builder import Sheet
...
from hw_agent.artifacts.schematics.interfaces import (
...
from hw_agent.artifacts.schematics.units import Resistance, Voltage
...
class Component:
    ...
    def _ensure_lib_loaded(self) -> None:
        if self._lib_pin_keys is not None:
            return
        from hw_agent.artifacts.schematics.kicad_lib import load_symbol
...
class Sheet:
    ...
    def write(self, path: Union[str, Path]) -> Path:
        ...
        if out.suffix == ".kicad_sch":
            from hw_agent.artifacts.schematics.ksa_writer import export_from_schematic
            schem = self.validate()
