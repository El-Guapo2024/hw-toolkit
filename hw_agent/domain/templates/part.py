...
from hw_agent.domain.templates._actuals_ops import merge as _merge_actuals
from hw_agent.domain.templates._actuals_ops import missing_fields
from hw_agent.domain.templates.buck import BuckActuals
from hw_agent.domain.templates.imu import ImuActuals
from hw_agent.domain.templates.ldo import LdoActuals
from hw_agent.domain.templates.mcu_ble import McuBleActuals
from hw_agent.domain.templates.motor_driver import MotorDriverActuals
from hw_agent.domain.templates.pwm_servo_driver import PwmServoDriverActuals
from hw_agent.domain.templates.stepper_driver import StepperDriverActuals
...
class Part:
    ...
    def _resolve_category(self) -> tuple[type, type]:
        ...
        if cat == "buck_converter":
            from hw_agent.domain.templates.buck import BuckSubsystem, BuckRequirements
            return BuckSubsystem, BuckRequirements
        if cat == "ldo":
            from hw_agent.domain.templates.ldo import LdoSubsystem, LdoRequirements
            return LdoSubsystem, LdoRequirements
        if cat == "imu":
            from hw_agent.domain.templates.imu import ImuSubsystem, ImuRequirements
            return ImuSubsystem, ImuRequirements
        if cat == "mcu_ble":
            from hw_agent.domain.templates.mcu_ble import McuBleSubsystem, McuBleRequirements
            return McuBleSubsystem, McuBleRequirements
        if cat == "motor_driver":
            from hw_agent.domain.templates.motor_driver import (
                MotorDriverSubsystem, MotorDriverRequirements,
            )
            return MotorDriverSubsystem, MotorDriverRequirements
        if cat == "stepper_driver":
            from hw_agent.domain.templates.stepper_driver import (
        ...
        if cat == "pwm_servo_driver":
            from hw_agent.domain.templates.pwm_servo_driver import (
