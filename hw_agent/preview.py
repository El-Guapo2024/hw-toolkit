...
Usage:
    python -m hw_agent.preview path/to/schematic.schem.json [--port 8765]
    python -m hw_agent.preview path/to/schematic.kicad_sch [--port 8765]
...
def pcb_check(source_path: Path, out_dir: Path) -> dict:
    ...
    import subprocess
    from hw_agent.schematics.pcb_writer import export_file as pcb_export
    from hw_agent.schematics.validators import validate, validate_pcb
...
def erc_check(source_path: Path, out_dir: Path) -> dict:
    ...
    """
    from hw_agent.schematics.eval import eval_from_json, run_eval, EvalResult
...
def kicad_sch_diff(json_path: Path, out_dir: Path) -> dict:
    ...
    """
    from hw_agent.schematics.sch_diff import parse_kicad_sch, diff
    ...
    if snap_path.exists():
        try:
            from hw_agent.schematics.sch_diff import Snapshot, SymbolSnap
